...
namespace eStore.BL.Reports.Accounts
{
    ...
    public class AccountReport
    {
        ...
        {
            var data = db.DailySales.Where (c => c.StoreId == StoreId && c.SaleDate.Month == onDate.Month && c.SaleDate.Year == onDate.Year).
                 Select(c=> new {c.InvNo, c.IsDue, c.IsManualBill,c.IsSaleReturn,c.IsTailoringBill,c.PayMode,c.Amount, c.SaleDate})
                .ToList ();

            var manul = data.Where (c => c.IsManualBill).ToList ();
            var saleReturn = data.Where (c => c.IsSaleReturn).ToList ();
            var onSale = data.Where (c => !c.IsManualBill && !c.IsSaleReturn && !c.IsTailoringBill).ToList ();
            var tail = data.Where (c => c.IsTailoringBill).ToList ();

        }

        public void PaymentRecieptReport(eStoreDbContext db, int storeId, DateTime onDate, bool isDdf = true)
        {
            var expdata = db.Expenses.Where (c => c.StoreId == storeId && c.OnDate.Month == onDate.Month && c.OnDate.Year == onDate.Year).ToList ();
            var payData = db.Payments.Where (c => c.StoreId == storeId && c.OnDate.Month == onDate.Month && c.OnDate.Year == onDate.Year).ToList ();
            var cashPayData = db.CashPayments.Where (c => c.StoreId == storeId && c.PaymentDate.Month == onDate.Month && c.PaymentDate.Year == onDate.Year).ToList ();
            var recptData = db.Receipts.Where (c => c.StoreId == storeId && c.OnDate.Month == onDate.Month && c.OnDate.Year == onDate.Year).ToList ();
            var cashRecptData = db.CashReceipts.Where (c => c.StoreId == storeId && c.InwardDate.Month == onDate.Month && c.InwardDate.Year == onDate.Year).ToList ();
