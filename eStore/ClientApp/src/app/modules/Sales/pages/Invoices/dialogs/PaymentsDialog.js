...
export default function PaymentsDialog({ id, show, onHide, payMode }) {
  ...
  const {
    ...
    (state) => ({
      ...
      paymentForEdit: state.invoices.paymentForEdit,
  ...
  useEffect(() => {
    ...
    dispatch(actions.fetchInvoice(id)); //Payment
...
export function DialogHeader({ id, paymentId }) {
  // Invoices Redux state
  const { invoiceForEdit, actionsLoading, paymentForEdit } = useSelector(
    (state) => ({
      invoiceForEdit: state.invoices.invoiceForEdit,
      paymentForEdit: state.invoices.paymentForEdit,
      actionsLoading: state.invoices.actionsLoading,
  ...
  useEffect(() => {
    ...
    if (invoiceForEdit && _title ==="New Payment") {
      _title = _title + `  For Sale Inv: ${invoiceForEdit.invNo}`;
    }
    if (invoiceForEdit && paymentForEdit && paymentId) {
      _title = `Payment for Sale '${invoiceForEdit.invNo}'`;
    }

    setTitle(_title);
    // eslint-disable-next-line
  }, [invoiceForEdit, actionsLoading, paymentForEdit]);
      ...
      <div className="m-4 p-2">
        <div className="row mb-3 mt-3 ">
          <div className="col-lg-4 text-primary">
            Invoice No: {invoice && invoice.invNo}
          </div>
          <div className="col-lg-4 text-primary">
            Date: {invoice && invoice.saleDate}
          </div>
          <div className="col-lg-4 text-primary">
            Mode: {invoice && payModes[invoice.payMode].name}
          </div>
        </div>
        <div className="row">
...
export function EditForm({
  ...
  const PaymentEditSchema = Yup.object().shape({
    ...
    remarks: Yup.string().required("Invoice details is required"),
  });
...
export function BankPaymentForm({
  ...
  payment.payMode = invoice ? invoice.payMode : 6;
                ...
                <div className="form-group row">
                  ...
                  <div className="col-lg-4">
                    <label className="text-primary">
                      On Date : {invoice && invoice.saleDate}
                    </label>
                  </div>
                  {/* Invoice No */}
                  <div className="col-lg-4">
                  ...
                  <div className="col-lg-4">
                    <label className="text-info">
                      Payment Mode :{" "}
                      {invoice && payModes[invoice.payMode].name}
                    </label>
                  </div>
                </div>
...
export function WalletPaymentForm({
  ...
  payment.payMode = invoice ? invoice.payMode : 9;
