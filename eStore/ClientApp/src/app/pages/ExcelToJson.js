...
export default function ExcelToJson() {
  ...
  const readExcelFile = (uMode) => {
    ...
    reader.onload = (e) => {
      ...
      const ws =
        sheetName && sheetName !== '' ? wb.Sheets[sheetName] : wb.Sheets[wsname]
...
export function ProcessImport() {
  ...
  const handleButtonClick = (e) => {
    const params={
      storeId:1,command:command,brand:brand,year:year
    };
    ProcessImports(params);
  }
  return (
    <>
      <div className="m-5">
        <h2>Process Import</h2>
        <div className="table-success p-5">
          <div className="h5 text-info">
            Year
          ...
          <div className="h5">
            Command
            <input type="text" id="commandName" onChange={(e) => setCommand(e.target.value)} />
          </div>
          <div className="h5">
            BrandName
            <input type="text" id="brandName" onChange={(e) => setBrand(e.target.value)} />
          </div>
...
export async function ProcessImports(command) {
  console.log(command);
  const api = BASE_URL + '/api/Importer/processVoyager'
  //const localApi="https://localhost:44385/api/Importer/voyagerImport";
  await axios
    .post(`${api}`, command, {
      method: 'POST',
